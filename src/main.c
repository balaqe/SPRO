...
void motor() {
  int count = 0;
  while (count > 255) {// 255 is the time period - for the pwm calculator

    if (TIFR1 & (1 << ICF1)) { // checks if there is a rising edge AKA the optocoupler has been interrupted, if so it gets the time 
...
int main(void)
{
  ...
  DDRD |= 0x60;
  OCR0A = 170; // pulse width - change this to controle speed
  // for testing
  //int count = 220;
  //OCR0A = 220;
  //int count2 = 0;
